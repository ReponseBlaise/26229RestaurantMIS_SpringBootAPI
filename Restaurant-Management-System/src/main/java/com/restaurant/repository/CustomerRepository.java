...
public interface CustomerRepository extends JpaRepository<Customer, Long> {
    ...
    @Query("SELECT DISTINCT c FROM Customer c WHERE c.village.parent.parent.parent.parent.name = :provinceName")
    ...
    @Query("SELECT DISTINCT c FROM Customer c WHERE c.village.parent.parent.parent.name = :districtName")
    Page<Customer> findByDistrict(@Param("districtName") String districtName, Pageable pageable);
    
    // Find customers by sector
    @Query("SELECT DISTINCT c FROM Customer c WHERE c.village.parent.parent.name = :sectorName")
    Page<Customer> findBySector(@Param("sectorName") String sectorName, Pageable pageable);
    
    // Find customers by cell
    @Query("SELECT DISTINCT c FROM Customer c WHERE c.village.parent.name = :cellName")
    Page<Customer> findByCell(@Param("cellName") String cellName, Pageable pageable);
    
    // Find customers by village
    @Query("SELECT DISTINCT c FROM Customer c WHERE c.village.name = :villageName")
